...
const Services: React.FC<ServicesProps> = ({ onOpenModal }) => {
  ...
  const rawItems = t('services_items', { returnObjects: true });
  const items = Array.isArray(rawItems) ? rawItems as { title: string; description: string }[] : [];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {SERVICES.map((service, index) => {
            const translated = items[index] ?? { title: service.title, description: service.description };
            return (
              <div
                key={index}
